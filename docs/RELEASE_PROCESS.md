...
| 数据库 schema | `v18` | `packages/server/database/migrations.ts` 最后一个迁移版本 |
| 数据库记录表 | `schema_migrations`、`app_upgrade_history` | 服务端首次启动时初始化或迁移 |

发布前必须确认：当前应用版本支持的目标 schema 版本明确、可初始化新库、可从上一发布版本迁移。

当前迭代为 0.2.7 - Unreleased，目标数据库版本为 v18。v16/v17 迁移保持冻结；v17 保存机构内项目及人工关联，v18 新增主动确认的自动关联规则及撤销决定，默认不启用、不批量关联历史记录。升级前自动备份，支持 v16/v17 直接升级至 v18。此次内部测试包不视为正式发布。旧开发期的 v17-v19 与本次迁移含义不同，按迁移标记和表结构区分，必须先经维护流程确认修复。

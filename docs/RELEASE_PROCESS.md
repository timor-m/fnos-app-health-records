...
**发布授权前提：**只有用户明确说“开始发版”或明确授权本次发布，才执行版本递增、schema 编号整理、冻结、版本提交、tag 和发布。开发、构建、安装测试不等于发布；正式发布后也不自动递增下一版本。开发阶段维护迁移草案，未发布改动在发布准备时统一收口；已落库测试数据不允许直接降级或删除。

本轮范围收缩：机构内管理界面、业务接口和自动关联已撤回。下述 v17/v18 表结构仅为已交付内部测试包的升级兼容保留，不表示该功能仍可使用；不降级、不删除用户数据。
...
当前迭代版本为 0.2.10（Unreleased，尚未冻结或发布），本轮包含成员共享与恢复身份的未编号结构草案，目标数据库版本保持 v17，本轮不新增迁移编号。补充记录、附件与趋势分组关联表通过启动期幂等补表应用（补表前自动备份），v17 及以前正式迁移不变；早期测试库缺表或带草案迁移记录时先备份再幂等补齐，不自动降号、删除历史或清库。内部测试包不视为正式发布。
...
- 开发者修改数据库结构时，维护本轮待发布迁移草案，不按功能逐次新增 schema 编号；用户明确开始发版后再统一整理迁移注册表。
...
3. 修改业务代码和数据访问代码。
4. 维护未发布迁移草案，不自动增加版本编号；保留已发布迁移及测试库兼容性。
5. 更新最新 schema。
6. 补充测试。
7. 更新文档和 changelog。
...
以下事务仅在用户明确开始发版后执行，不因一次构建或测试自动触发：
...
2. `CHANGELOG.md`
   - 先在当前未发布章节写清楚变更内容。
   - 正式发布后版本章节冻结为 `## x.y.z`；后续开发使用无编号 `Unreleased`，下一次发布授权后再编号，不沿用已发布版本条目。
   - 写明是否包含数据库迁移。
...
Issue #40 保留未编号迁移草案。发布前复核新库、已有 v17 库、同实例备份恢复和未完成补充批次的存储迁移；真实 fnOS/Docker 分别验收 PDF 多页、照片、授权目录及外部 OCR/AI 故障重试。不能自动合并/删除重复健康数据或替恢复账号扩大权限。当前实现与保守边界见 [补充报告页](REPORT_PAGE_APPEND.md)。

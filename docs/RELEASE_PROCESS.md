...
当前迭代为 0.2.7 - Unreleased，目标数据库版本为 v17。v16 及以前正式迁移不变，本轮原 v17/v18 草案合并为 v17，保留兼容表与上传回执结构，不重新启用已撤回的关联功能。v16 升级前自动备份；已有 v17 测试库缺表时先备份再补齐。此前带机构项目迁移标记的 v18 测试库仍触发高版本保护，不自动降号、删除历史或清库，需单独处理。内部测试包不视为正式发布。

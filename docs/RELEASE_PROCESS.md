...
当前封板版本为 0.2.9（尚待发布），相对 0.2.8 无新增 schema 变化，目标数据库版本保持 v17，本轮不新增迁移编号。补充记录、附件与趋势分组关联表通过启动期幂等补表应用（补表前自动备份），v17 及以前正式迁移不变；早期测试库缺表或带草案迁移记录时先备份再幂等补齐，不自动降号、删除历史或清库。内部测试包不视为正式发布。

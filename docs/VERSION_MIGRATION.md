...
当前发布为 0.2.8，目标数据库版本保持 **v17**，本轮不新增迁移编号。补充记录（`report_notes`）、补充附件（`report_note_assets`）和趋势分组关联表（`indicator_groups`、`indicator_group_members`）通过启动期幂等补表应用，补表前自动备份；v17 及以前正式迁移不变，早期测试库缺表时先备份再幂等补齐。

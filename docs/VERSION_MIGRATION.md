...
当前目标数据库版本为 **v17**（应用版本仍为 0.2.7 Unreleased）。本轮原 v17/v18 草案合并为 v17，包含机构项目兼容表、规则兼容表和 `upload_receipts`。v16 升级前自动备份；早期 v17 测试库缺表时先备份再幂等补齐。正式 v16 及以前迁移保持不变。

本轮已撤回机构内关联 UI、接口与自动规则执行逻辑；兼容表保留，不主动清理已有项目或规则数据。此前机构项目版 v18 测试库高于当前目标，会拒绝启动，必须单独处理；禁止自动删除迁移记录或清库。

当前健康档案应用已经建立：

- 此前内部测试包曾使用 v18，本次按用户要求在正式发布前收口为 v17。v16 已包含提取与治理表；v17 合并本轮新增结构。保留旧 v17 标记以识别测试库来源，补表前备份，不调用外部 AI；真正的未来版本仍拒绝启动。
...
后续数据库结构变更必须维护迁移草案及测试，不自动追加编号；仅在用户明确开始发版后统一处理版本事务。此前误增的 v19 已从代码中撤回；不自动修改真实数据库的迁移记录。若其他测试环境已运行过 v19，应先备份并单独确认兼容处理，不能直接降号或清库。

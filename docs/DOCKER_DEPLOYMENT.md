...
管理员也可以在“我的 -> 账号安全 -> 本地账号管理”添加普通本地账号。新账号的临时密码统一为 `admin`，首次登录同样会强制修改密码。管理员重置普通账号时也会恢复为该临时密码。所有本地账号（含管理员）都可以修改显示名称与用户名；修改用户名不影响当前登录状态，下次登录需使用新用户名，显示名称仅影响界面展示，不影响同名成员档案。普通账号支持停用与启用：停用会立即撤销其登录会话并禁止登录，成员档案与健康数据保留。普通账号也可删除，删除仅移除登录身份且不可恢复，成员档案与健康数据同样保留；若该账号名下存在仅其管理的成员档案且包含报告，删除前会要求再次强制确认，强制删除后这些数据将无法被任何账号访问但仍占用存储，建议先清理或授权后再删除。以上账号仅存在于 Docker/本地模式，fnOS 应用模式继续使用 fnOS 网关账号和密码，不提供这套账号管理。
...
Compose 默认把 `docker-compose.yml` 同目录下的 `./reports` 只读挂载到容器 `/reports`。把报告放入该目录后，以管理员登录，进入“上传报告”，点击“从 NAS 导入”即可浏览和选择。列表会按需显示图片缩略图，点击可放大查看，PDF 可预览第一页。支持 HEIC、JPEG、PNG、WebP 和 PDF；一次最多 1000 个文件，单个文件最大 40 MB，合计最大 2 GB；支持独立报告、合并报告及自定义分组，详见[批量导入说明](./REPORT_IMPORT.md)。
...
Docker/本地模式恢复备份（应用内恢复或上传恢复）出于安全只保留当前管理员的登录凭据：恢复完成后，其他本地账号会被自动停用，其数据不丢失，需要时在“我的 -> 账号安全”中重新启用即可；恢复完成提示和恢复前的确认弹窗中都会说明被停用的账号数量。
